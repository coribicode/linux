...
apt-get upgrade -qqy  2>/dev/null
systemctl daemon-reload  2>/dev/null
apt-get --fix-broken -qq install  2>/dev/null
echo "[ Repositório - WINEHQ ]: - OK!"
fi
...
sudo -u $USER WINEPREFIX="$winedir/.wine" wine winecfg -v win10 2>/dev/null
